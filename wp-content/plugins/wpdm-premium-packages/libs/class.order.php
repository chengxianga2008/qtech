<?php
    class Order{
        ...
        static function add_note($id, $data, $type = 'messages'){
            ...
            //if(isset($data['seller'])) wp_mail($customer->user_email, "New Note: Order# {$id}", $data['note'], "From: {$fromname} <{$frommail}>\r\nContent-type: text/html\r\n");
        ...
        static function complete_order($id, $email_notify = true, $payment_method = null){
            ...
            $buyer_email = $user_info->user_email;
                ...
                $buyer_email = get_option("email_".$id);
                if($buyer_email=='' && $payment_method){
                    $buyer_email = $payment_method->buyer_email;
                }
            ...
            if($guest_checkout==1)
                $message = file_get_contents(WPDMPP_BASE_DIR.'email-templates/wpdm-pp-new-order-guest.html');
            else
                $message = file_get_contents(WPDMPP_BASE_DIR.'email-templates/wpdm-pp-new-order-guest.html');
        ...
        static function UpdateOrderItems($cart_data, $id){
            ...
            if(!empty($cart_data))
                foreach($cart_data as $pid=>$cdt){
                    ...
                    $wpdb->insert("{$wpdb->prefix}ahm_order_items",array('oid'=>$id,'pid'=>$pid,'quantity'=>$quantity,'price'=>$price,'variations'=>  serialize($vrts),'coupon'=>$coupon,'coupon_discount'=>floatval($coupon_amount)));
            }
        }
