...
$pluginurl=plugins_url("wpdm-premium-packages/images/");
...
if($count%2==1) $trclass="odd";else $trclass="even";
    ...
    //print_r(get_post_meta($oitem->pid,"wpdmpp_list_opts",true));
     ...
     @extract(get_post_meta($oitem->pid,"wpdmpp_list_opts",true));
...
if($order->payment_status!='Completed'){
    ...
    <tr class="items"><td colspan="4">Please complete your payment to view download links. <div id="proceed_{$order->order_id}" style="float:right">    
         ...
         function proceed2payment_{$order->order_id}(ob){
            ...
            jQuery.post('{$purl}',{task:'paymentfront',action:'wpdmpp_ajax_call',execute:'PayNow',order_id:'{$order->order_id}',payment_method:jQuery('#pgdd_{$order->order_id}').val()},function(res){
...
window.print();
  function getkey(file, order_id){
      ...
      jQuery.post('{$homeurl}',{action:'wpdmpp_ajax_call',execute:'getlicensekey',fileid:file,orderid:order_id},function(res){
