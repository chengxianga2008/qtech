<?php
    ...
    function wpdmpp_reduce_stock($orderid=0){
        ...
        if($settings['stock']['enable']==1){
            ...
            foreach($order_items as $product_info){
                $post_meta=array();
                $post_meta=get_post_meta($product_info['pid'],"wpdmpp_list_opts",true);
                //print_r($post_meta);
                if($post_meta['manage_stock']==1){
                    ...
                    update_post_meta($product_info['pid'],"wpdmpp_list_opts",$post_meta);
                }
            }
            die("Stock Reduced");
        }else{
    ...
    function wpdmpp_restore_stock($orderid=0){
        ...
        if($settings['stock']['enable']==1){
            ...
            foreach($order->items as $pid=>$product_info){
                ...
                update_post_meta($pid,"wpdmpp_list_opts",$post_meta);
            }
            die("Stock Restored");
        }else{
            die("Stock not enabled");
    ...
    function wpdmpp_get_stock($productid=0){
        $settings = maybe_unserialize(get_option('_wpdmpp_settings'));
        if($settings['stock']['enable']==1){
