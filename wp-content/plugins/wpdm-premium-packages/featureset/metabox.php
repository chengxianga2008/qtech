...
$nonce = wp_create_nonce("wpdmpp_featurelist_nonce");

$data1 = get_post_meta($post->ID, "_wpdmpp_featureset", true);
             ...
             <tr>
                ...
                if($row['field_type']=="radio" || $row['field_type']=='checkbox'){
                    ...
                    foreach($opt_value as $a => $b):
                        ...
                        echo "<tr><td><input type='".$row['field_type']."' name='wpdmpp_featureset[$id][$key][]' value='$b' $checked > $b </td></tr>"; 
                    endforeach; 
                    echo "</table></td>";
                }
                else if($row['field_type']=="dropdown"){
                    echo "<td><select name='wpdmpp_featureset[$id][$key][]'>";
                     foreach($opt_value as $a => $b):
                        if(in_array($b, $value)){
                    ...
                    //$data = str_replace("<br>", "\n", $value[0]);
                    $data = $value[0];
                    echo "<td><textarea name='wpdmpp_featureset[$id][$key][]' rows='5' cols='50'>$data</textarea></td>";
        ...
        $('#featurelist_select').live("change",function(){
            ...
            if(id != ""){
                 ...
                 jQuery.ajax({
                    ...
                    dataType : "json",
                    url : ajaxurl,
                    data : {action: "wpdmpp_featurelist_generate", id : id, nonce: nonce},
            ...
            $.each(response.data, function(key,value){
                ...
                if(value.field_type=="radio" || value.field_type=="checkbox"){
                    ...
                    $.each(value.data,function(a,b){
                        str += '<tr><td>';
                        str += '<input type="'+value.field_type+'" name="wpdmpp_featureset['+id+']['+value.id+'][]" value="'+b+'"/> ';
                        str += b+'</td></tr>';
                    });
                    str += '</table></td>';
               }
               else if(value.field_type=="dropdown"){
                    str += '<td><select name="wpdmpp_featureset['+id+']['+value.id+'][]">';
                    $.each(value.data,function(a,b){
                   ...
                   str += '<td><textarea name="wpdmpp_featureset['+id+']['+value.id+'][]" rows="5" cols="50">';
                   $.each(value.data,function(a,b){
                       str += "<p>" + b +'<p>\n';
                   });
                   str += "</textarea></td>";
