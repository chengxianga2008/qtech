...
//    if($dashboard){
//        $zurl = $orderurl . $sap."section=my-orders&";
//    }
//    else {
//        $zurl = $orderurl . $sap;
//    }
    ...
    //$link = admin_url('admin-ajax.php?action=wpdmpp_delete_frontend_order&id='.$order->order_id.'&nonce='.$nonce);
...
$odetails   = __("Purchases","wpmarketplace");
...
$licns      = __("License","wpmarketplace");
...
<div class="panel panel-default panel-purchases">
<div class="panel-heading"><b><a href="{$link}" class="pull-left">$odetails</a> &nbsp;<i class="fa fa-angle-double-right"></i>  &nbsp;{$order->title} </b><span class="pull-right"><a class="btn btn-info btn-xs white btn-invoice" href="#" onclick="window.open('?id={$order->order_id}&amp;wpdminvoice=1','Invoice','height=720, width = 750, toolbar=0'); return false;"><i class="fa fa-bars"></i> Invoice</a></span></div>
<div>
...
LIC;
                ...
                if (count($files) > 1 && $order->order_status == 'Completed') {
                    ...
                    foreach ($files as $ff) {
                        ...
                        $title = $data[$ff]['title'] ? $data[$ff]['title'] : basename($ff);
...
ITEM;
        ...
        $vdlink_expired = sprintf(__("If you want to get continuous support and update for another %d days", "wpmarketplace"), $wpdmpp_settings['order_validity_period']);
        $pnow = __("Pay Now", "wpmarketplace");
        $pnow_expired = __("Renew Now", "wpmarketplace");
