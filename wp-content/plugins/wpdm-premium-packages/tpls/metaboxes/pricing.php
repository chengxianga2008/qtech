...
<div class="panel panel-default">
    <div class="panel-heading"><?php echo __('Pricing','wpmarketplace'); ?></div>
    <div class="panel-body">
        <div class="row">
            <div class="col-md-6">
                <label for="base-price-field"><?php echo __('Base Price','wpmarketplace'); ?></label>
            ...
            <div class="col-md-6">
                <label for="base-price-field"><?php echo __('Sales Price','wpmarketplace'); ?></label>
    ...
    <div class="panel-heading">
        <label><input style="margin: 0;line-height: 10px" type="checkbox" <?php if($price_variation!='') echo "checked='checked'"; else echo "";?> name="file[price_variation]" id="price_variation" name="price_variation" > <?php echo __('Activate Variable Pricing','wpmarketplace'); ?></label>
    </div>
                ...
                if(is_array($variation)){
                    foreach($variation as $key=>$vname){?>
                        <div id="variation_div_<?php echo $key;?>" class="panel panel-default">
                            <div class="panel-heading">
                                <?php _e('Variation ID#','wpmarketplace');  ?> <?php echo $key;?>
                            ...
                            <table class="table table-v" id="voption_table_<?php echo $key;?>">
                                <tr>
                                    <td colspan="3"><label><input style="margin: 0" type="checkbox" name="file[variation][<?php echo $key;?>][multiple]" placeholder="Multiple Select" <?php if(isset($vname['multiple'])) echo "checked='checked'"; ?> > &nbsp;<?php echo __('Multiple Select','wpmarketplace'); ?></label></td>
                                </tr>
                                <tr><td colspan="3"><input class="form-control" type="text" name="file[variation][<?php echo $key;?>][vname]" id="" placeholder="<?php _e('Variation Name','wpmarketplace');  ?>" title="<?php _e('Enter a Variation Name','wpmarketplace');  ?>" value="<?php echo $vname['vname'];?>"></td></tr>
                                <tr><th><?php _e('Option Name','wpmarketplace'); ?></th><th><?php _e('Extra Cost','wpmarketplace'); ?></th><th><?php _e('Delete','wpmarketplace'); ?></th></tr>
                                <?php
                                if($vname){
                                    foreach($vname as $optionkey=>$optionval){
                                        if($optionkey!="vname" && $optionkey != "multiple"){?>
                                            <tr id="voption<?php echo $optionkey;?>"><td><input type="text" name="file[variation][<?php echo $key;?>][<?php echo $optionkey;?>][option_name]"  placeholder="Option Name" class="form-control input-sm" value="<?php echo $optionval['option_name'];?>"></td><td><div class="input-group input-group-sm"><span class="input-group-addon"><i class="fa fa-plus-circle"></i></span><input style="max-width: 70px" min="0" name="file[variation][<?php echo $key;?>][<?php echo $optionkey;?>][option_price]" id="" size="5" class="form-control" type="number" placeholder="price" value="<?php echo $optionval['option_price'];?>"></div></td><td><i class="delet_voption fa fa-times-circle text-danger" rel="voption<?php echo $optionkey;?>" title="delete this option" src="<?php echo plugins_url("wpdm-premium-packages");?>/images/remove.png" alt="" style="cursor:pointer"></i></td></tr>
                                        <?php
                                        }
                                    }
                                }
                            ...
                            <input type="button" class="btn btn-default btn-sm add_voption" rel="<?php echo $key;?>" value="Add Option">
                            </div>
                        </div>
                    <?php
                    }
    ...
    <div class="panel-footer">
        <input type="button" class="btn btn-primary" id="add_variation" value="Add Variation">
    </div>
    </div>
      ...
      <div class="panel panel-default">
        <div class="panel-heading"><?php _e('Licensing Option','wpmarketplace'); ?></div>
        <div class="panel-body">
            <b><label><input type="checkbox" style="margin: 0 !important;" value="1"   name="file[enable_license]" <?php if(get_post_meta($post->ID, "__wpdm_enable_license", true)==1) echo 'checked="checked"'; ?> > &nbsp;<?php _e('License Key Required','wpmarketplace'); ?></label></b>
        </div>
        </div>
        

    </div><div class="col-md-6  wpdm-full-front">
      <div id="wpdmpp_discount">
        ...
        ?>
        <tr><td><?php echo $name; ?> (<?php echo $role; ?>) </td><td><input class="form-control input-sm" style="width: 70px" type="text" size="8" name="file[discount][<?php echo $role; ?>]" value="<?php if(isset($discount[$role])) echo $discount[$role]; ?>"></td></tr>
        ...
        <?php } ?>
        <div class="panel panel-default">
        <div class="panel-heading"><?php _e('Coupon Discount','wpmarketplace'); ?></div>
         <table id="coupon_table" class="table table-v">
             <thead>
         <tr><th align="left"><?php _e('Coupon Code','wpmarketplace'); ?></th><th align="left" style="width: 80px"><?php _e('Discount(%)','wpmarketplace'); ?></th><th style="width: 50px"><?php _e('Delete','wpmarketplace'); ?></th></tr>
             </thead>
             <tbody>
         <?php
        ...
        if(is_array($coupon_code) && count($coupon_code)>0){
            ...
            ?>
         <tr><td> <input class="form-control input-sm" type="text" size="8"  name="file[coupon_code][<?php echo $coupon_key?>]"  value="<?php echo $coupon_code[$coupon_key];?>"></td><td><input class="form-control input-sm" type="text" size="8" name="file[coupon_discount][<?php echo $coupon_key?>]"  value="<?php echo $coupon_discount[$coupon_key];?>"></td><td><i class="fa fa-times-circle"></i></td></tr>
              <?php
            }
        }
    ?>
         ...
         <tfoot>
         <tr><td><input type="text" class="form-control input-sm" size="16" id="coupon_code"  value=""></td><td><input class="form-control input-sm" type="text" size="8" id="coupon_discount"  value=""></td><td><input class="btn btn-default btn-sm" type="button" size="8" id="add_coupon" value="Add"></td></tr>
         </tfoot>
         </table>
            <script type="text/javascript">
