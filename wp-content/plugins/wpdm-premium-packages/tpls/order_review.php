...
if($settings['calc_shipping']==1){
    $ship=wpdmpp_calculate_shipping();
...
$tax_summery=$order->wpdmpp_calculate_tax();
             ...
             if($item['variation']){
                ...
                foreach($item['variation'] as $var){                   
                    if($var==$optionkey){
                        ...
                        $variations.=$optionvalue['option_name'].": ".number_format(floatval($optionvalue['option_price']),2,".","")." ";
...
$extra_row = '';
$cart .= apply_filters('wpdmpp_ckeckout_extra_row',$extra_row);

$cart .= "

<tr><td colspan=5 align=right>".__("Cart Subtotal:","wpmarketplace")."</td><td class='amt' id='wpdmpp_cart_subtotal'>".$currency_sign.wpdmpp_get_cart_subtotal()."</td></tr>

".$tax_row."
<tr><td colspan=5 align=right>".__("Discount:","wpmarketplace")."</td><td class='amt' id='wpdmpp_cart_discount'><input type='hidden' name='cart_discount' value='".wpdmpp_get_cart_discount()."'>".$currency_sign.wpdmpp_get_cart_discount()."</td></tr>
