...
OTH;
    ...
    foreach ($items as $itemid) {
        ...
        //$v_price = wpdmpp_cart_item_var_price($cart_data[$itemid]);
        $v_name = '';// wpdmpp_cart_item_var_name($cart_data[$itemid]);
        //dd($v_price);
        //$netsubtotal += number_format($cart_data[$itemid]['quantity'] * $v_price, 2);
...
ITEM;
    ...
    <div class="fn">{$first_name} {$last_name}</div>
    <div class="org"><h3>{$company_name}</h3></div>
    <div class="adr"> 
      ...
      <!-- street-address --> 
      <div class="locality">{$city}, {$country}</div> 
      <div id="client-postcode"><span class="region"></span> <span class="postal-code">{$phone}</span></div> 
      <div id="your-tax-number">{$current_user->user_email}</div>
    </div> 
    <!-- adr --> 
  </div>
...
$data = file_get_contents(WPDMPP_BASE_DIR . '/invoice-template/invoice.html');
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo WPDM_BASE_URL; ?>css/front.css" />
    <style>
        .w3eden{
...
<div class="container-fluid">
 <br/>
    ...
    <div class="row frow">
        <div class="col-xs-<?php echo isset($_GET['renew'])?4:6; ?>">
            <div class="panel panel-default"><div class="panel-heading">
                    <button class="btn btn-primary btn-xs pull-right" id="btn-print" type="button" style="margin-top: -5px;margin-right: -8px" onclick="window.print();"><i class="fa fa-print"></i> Print Invoice</button><strong>Invoice No</strong>
                </div>
    ...
    <div class="row">
        <div class="col-xs-6">
            <div class="panel panel-default info-panel"><div class="panel-heading">
            <strong>From:</strong>
                </div>
                <div class="panel-body">
            <?php if($settings['invoice_logo'] != ""){ ?>
            <img style="max-width: 100%;margin-bottom: 6px" src="<?php echo $settings['invoice_logo']; ?>" />
            <?php } else { ?>
            <h3><?php bloginfo('sitename'); ?></h3>
            <?php }
            echo nl2br($settings['invoice_company_address']);
            ?>
                </div>
            </div>
