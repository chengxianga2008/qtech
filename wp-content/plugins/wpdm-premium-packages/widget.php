...
class WPDMPP_MiniCart extends WP_Widget {
    ...
    function reload(){
        ...
        echo json_encode($shopping_cart); die();
    }
       ...
       $shopping_cart=load_ajax_cart();
        ...
        <style>
            ...
            #wpdm-cart-panel-trigger {
                ...
                background: #ffffff url("<?php echo WPDMPP_BASE_URL ?>images/shoppingbag.png") no-repeat 87px center;
    ...
    function form( $instance ) {
        ...
        <p>
        <label for="<?php echo $this->get_field_id('panel'); ?>"><input class="widefat" id="<?php echo $this->get_field_id('panel'); ?>" name="<?php echo $this->get_field_name('panel'); ?>" type="checkbox" value="1" <?php echo checked($panel, 1); ?>" /> <?php _e('Show as Panel'); ?></label>
        </p>
       ...
       $currency_sign = wpdmpp_currency_sign();
        ...
        if(is_array($cart_items) && count($cart_items)>0) {
            ...
            foreach ($cart_items as $item) {
                ...
                $variations = "";
                /*
                @extract(get_post_meta($item['ID'],"wpdmpp_list_opts",true));
                $svariation = array() ;
        ...
        <div class='media'><button class='btn btn-primary btn-sm btn-block' type='button' onclick='location.href=\"" . get_permalink($settings['page_id']) . "\"'><span id='wpdmpp_mini_cart_subtotal'>" . $currency_sign . wpmpp_get_cart_total() . "</span> &nbsp; | &nbsp; " . __("Checkout", "wpmarketplace") . "</button></div>
        ...
        ";
            ...
            $cart_['content'] = "<div class='panel panel-default'><div class='panel-body text-danger'>". __('Cart is Empty !','wpmarketplace')."</div><div class='panel-footer'><a href='".wpdmpp_continue_shopping_url()."' class='btn btn-sm btn-primary'>".__('Continue Shopping','wpmarketplace')."</a></div></div>
