...
function ihc_native_user_field(){
	...
	$arr = array(
			...
			array( 'display_admin'=>0, 'display_public_reg'=>0, 'display_public_ap'=>0, 'name'=>'payment_select', 'label'=>'Select Payment', 'type'=>'payment_select', 'native_wp' => 0, 'req' => 0, 'sublevel' => '' ),
	);
...
function ihc_insert_update_transaction($u_id, $txn_id, $post_data, $dont_save_order=FALSE){
	...
	if ($exists){
		...
		$q = $wpdb->prepare("INSERT INTO $table VALUES (null, %s, %d, %s, %s, null, null);", $txn_id, $u_id, $post_data, $history_str);
		$wpdb->query($q);
	}
...
function ihc_insert_update_order($uid=0, $lid=0, $amount_value=0, $status='pending'){
	/*
...
function ihc_print_bank_transfer_order($u_id, $l_id){
	...
	if (!empty($discount_type) && !empty($discount_value)){
		$msg = ihc_replace_price_for_bt_with_coupon($msg, $l_id, $discount_type, $discount_value);
	}
...
function ihc_replace_price_for_bt_with_coupon($msg, $l_id, $discount_type='', $discount_value=0){
	/*
...
function ihc_get_user_id_by_user_login($u_login=''){
	...
	if (!empty($u_login)){
		global $wpdb;
		$q = $wpdb->prepare("SELECT ID FROM " . $wpdb->prefix . "users WHERE user_login=%s ;", $u_login);
		$data = $wpdb->get_row($q);
		if (!empty($data->ID)){
			return $data->ID;
		}
