...
if (!class_exists('IndeedFeedSystem')){
	class IndeedFeedSystem{
		...
		private $option_name_data = 'indeed_feed_data';
		...
		private $dir_path = '';
		private $dir_url = '';
		private $menu_style = 'color:#fff; background: #d54e21;     padding: 4px 16px 4px 13px; margin: 0 0 0 -13px; width: 100%; display: block;';
		...
		public function output(){
			...
			if (!empty($data) && is_array($data) && !empty($data['items']) && !empty($data['settings']) && !empty($data['cats'])){
				$items = $this->do_reorder($data['items']);
		...
		private function do_update(){
			...
			if (wp_remote_retrieve_response_code($response)==200 && !is_wp_error($response) && is_array($response) && !empty($response['body'])){
				...
				if (!empty($data) && is_array($data) && !empty($data['items']) 
					&& !empty($data['cats']) && !empty($data['settings']) 
		...
		public function do_notification(){
			...
			if (!empty(self::$current_notification) && !empty(self::$current_notification['message']) && isset(self::$current_notification['index'])){
				...
				$output .= '</script>';
				$output .= '<div class="updated fade" id="indeed_main_notify" style="padding: 10px;">';
				$output .= '<div style="display: inline-block;width: 80%;">' . self::$current_notification['message'] . '</div>';
				$output .= '<a href="' . $this->admin_url  . '">List of all Azzaroco Items</a>';
				$output .= '<div style="display: inline-block;vertical-align: top;text-align:right;cursor: pointer;margin-left: 10px;" onClick="indeed_close_notf_div();"><i class="fa-ifs ifs-close"></i></div>';
				$output .= '</div>';
				echo $output;				
			}
			return '';
		...
		public function print_global_style(){
			...
			$output .= '#toplevel_page_indeed_extensions_plus:hover{color: #FFF !important;}';
			$output .= '#toplevel_page_indeed_extensions_plus:hover div.wp-menu-image:before{color: #FFF !important;}'; 
