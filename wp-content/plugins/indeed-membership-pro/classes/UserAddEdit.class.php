	...
	class UserAddEdit{
			...
			if($paid == 1){		
				...
				if ($this->payment_gateway=='authorize'){//ihcpay
					...
					if (isset($level_data['access_type']) && $level_data['access_type']=='regular_period'){
						...
						///Save Order 
						$this->insert_the_order('Completed');
					}
				}
						...
						switch ($this->payment_gateway){
							...
							case 'twocheckout':	
								...
								if ( ihc_check_payment_available('twocheckout') ){	
									$this->insert_the_order();
									ihc_twocheckout_submit($this->user_id, $lid, $this->coupon);//function available in utilities.php
								} else {
									$redirect_back = TRUE;
								}								
								break;
						...
						switch ($this->payment_gateway){
							...
							case 'twocheckout':
								if ( ihc_check_payment_available('twocheckout') ){
									/// SAVE THE ORDER
									$this->insert_the_order();																	
									ihc_twocheckout_submit($this->user_id, $_POST['lid'], $this->coupon);//function available in utilities.php
								}
								break;
							case 'bank_transfer':
								if ( ihc_check_payment_available('bank_transfer') ){
		...
		private function insert_the_order($status='pending'){
			...
			if (!empty($this->user_id) && $lid!==FALSE){
				...
				$order_id = ihc_insert_update_order($this->user_id, $lid, $amount, $status);				
			}
		}		
