...
define("CC_WHMCS_BRIDGE_VERSION","3.7.2");
...
function cc_get_cache() {
    $current = unserialize(get_option('cc_whmcs_bridge_cache'));
...
function cc_update_cache($url, $cached_filename) {
    $current = cc_get_cache();
    $current[$url] = $cached_filename;
    update_option('cc_whmcs_bridge_cache', serialize($current));

    cc_whmcs_log(0, 'Updating cache {'.$url.'} {'.$cached_filename.'} - '.print_r($current, true));
}
