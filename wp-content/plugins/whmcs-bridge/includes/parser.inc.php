...
function cc_whmcs_bridge_parser_ajax1($buffer, $page_to_include = '') {
    ...
    if (stristr($page_to_include, 'cart') !== false || stristr($page_to_include, 'quantumvault') !== false) {
...
function cc_whmcs_bridge_parser($buffer=null,$current=false) {
    ...
    if ($parse) {
        ...
        foreach ($loop as $rep_url) {
            ...
            // templates css/js
            $f[] = "/src=\"" . preg_quote($rep_url, '/') . "templates\/([a-zA-Z0-9\_]*?)\/js\/([a-zA-Z0-9]*?).js/";
            $r[] = "src=\"{$home}?ccce=js&ajax=1&js=" . 'templates/$1/js/$2.js' . $pid;

            $f[] = "/href=\"" . preg_quote($rep_url, '/') . "templates\/([a-zA-Z0-9\_]*?)\/css\/([a-zA-Z0-9]*?).css/";
            ...
            */
            ...
            $f[]="/src=\"".preg_quote($rep_url,'/')."templates\/orderforms\/([a-zA-Z0-9\_]*?)\/js\/([a-zA-Z0-9]*?).js/";
            $r[]="src=\"{$home}?ccce=js&ajax=1&js=".'templates/orderforms/$1/js/$2.js'.$pid;

            $f[]="/src=\"".preg_quote($rep_url,'/')."templates\/orderforms\/([a-zA-Z0-9\_]*?)\/([a-zA-Z0-9]*?).js/";
            $r[]="src=\"{$home}?ccce=js&ajax=1&js=".'templates/orderforms/$1/$2.js'.$pid;

            $f[]="/href=\"".preg_quote($rep_url,'/')."templates\/orderforms\/([a-zA-Z0-9\_]*?)\/([a-zA-Z0-9]*?).css/";
            $r[]="href=\"{$home}?ccce=js&ajax=1&js=".'templates/orderforms/$1/$2.css'.$pid;

            $f[]="/href=\"".preg_quote($rep_url,'/')."templates\/orderforms\/([a-zA-Z0-9\_]*?)\/css\/([a-zA-Z0-9]*?).css/";
    ...
    $buffer = str_replace('action="modules', 'action="' . $home . '?ccce=js&ajax=2&js=modules', $buffer);
    // six
