...
if($roleManage['front_ticket_all'] || count($allowed_roles)>0)
{
    if($advancedSettings['logout_Settings']==1){?>
        <div id="wpsp_user_welcome">
            <?php
            echo(__("Welcome", 'wp-support-plus-responsive').' <b>' . $current_user->display_name .'</b>'.'.');
            ?>
            <?php wp_loginout($_SERVER['REQUEST_URI']); ?>
        </div><br>
    <?php }?>
    <ul class="nav nav-tabs">
        <li class="active"><a href="#" onclick="wpsp_getAllTickets();" id="tab_ticket_container" data-toggle="tab"><?php echo __($advancedSettings['ticket_label_alice'][2], 'wp-support-plus-responsive');?></a></li>
        <?php
        /* BEGIN CLOUGH I.T. SOLUTIONS MODIFICATION
         * Update 10 - disable front end new ticket submission
        ...
        if ( $generalSettings['front_end_submission'] == 0 ) {
            ?>
            <li><a href="#" onclick="wpsp_getCreateTicket();" id="tab_create_ticket" data-toggle="tab"><?php echo __($advancedSettings['ticket_label_alice'][3], 'wp-support-plus-responsive');?></a></li>
            <?php
        }
        /* END CLOUGH I.T. SOLUTIONS MODIFICATION
         */
        ?>
        <li><a href="#" onclick="wpsp_getFAQ();" id="tab_faq" data-toggle="tab"><?php echo __("FAQs", 'wp-support-plus-responsive');?></a></li>
    </ul>
    <!-- Tab panes -->
