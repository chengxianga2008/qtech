...
$et_success_staff_subject='['.__($advancedSettings['ticket_label_alice'][1],'wp-support-plus-responsive').' '.$advancedSettings['wpsp_ticket_id_prefix'].$_POST['ticket_id'].'] '.stripslashes($wpsp_et_change_ticket_assign_agent['mail_subject']);
...
$previously_assigned=__('None','wp-support-plus-responsive');
...
$nowAssigned=__('None','wp-support-plus-responsive');
...
foreach ($wpsp_et_change_ticket_assign_agent['templates'] as $et_key=>$et_val){
    switch ($et_key){
        case 'ticket_status':
            $et_success_staff_subject = str_replace('{ticket_status}', __(ucfirst($ticket->status),'wp-support-plus-responsive'), $et_success_staff_subject);
            $et_staff_body = str_replace('{ticket_status}', __(ucfirst($ticket->status),'wp-support-plus-responsive'), $et_staff_body);
            break;
        case 'customer_name':
        ...
        case 'ticket_priotity':
            $et_success_staff_subject = str_replace('{ticket_priotity}', __($ticket->priority,'wp-support-plus-responsive'), $et_success_staff_subject);
            $et_staff_body = str_replace('{ticket_priotity}', __($ticket->priority,'wp-support-plus-responsive'), $et_staff_body);
            break;
        case 'updated_by':
...
if($wpsp_et_change_ticket_assign_agent['notify_to']['administrator']){
    foreach ($adminEmails as $adminEmail){
        if(!(array_search($adminEmail, $piping_emails)>-1)){
...
foreach ($supervisors as $supervisor){
    if($supervisor->ID!=$current_user->ID && !(array_search($supervisor->user_email, $piping_emails)>-1)){
        $headers[] = " Bcc:" . $supervisor->user_email;
    }
}
