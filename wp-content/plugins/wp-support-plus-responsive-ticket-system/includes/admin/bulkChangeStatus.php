  ...
  <tr>
    ...
    <td>
    	<select id="change_status_ticket_status">
			<option value="select"><?php _e('Select','wp-support-plus-responsive');?></option>
			<?php
				...
				if($total_statusses)
				{
					foreach($custom_statusses as $custom_status){?>
						<option value="<?php echo $custom_status->name?>"><?php _e(ucfirst($custom_status->name),'wp-support-plus-responsive');?></option>
					<?php
					}
				}
			?>
...
<button class="btn btn-success" onclick="wpspBulkChangeStatusSubmitChanges();"><?php _e('Save Changes','wp-support-plus-responsive');?></button>
