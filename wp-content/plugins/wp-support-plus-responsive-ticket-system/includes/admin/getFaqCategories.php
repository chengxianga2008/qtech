...
<div id="faqCatDisplayTableContainer" class="table-responsive">
	<table class="table table-striped">
		...
		<?php foreach ($faqcategories as $faqcategory){?>
			<tr>
				<td><?php _e($faqcategory->name,'wp-support-plus-responsive');?></td>
				<td>
...
<input id="newFaqCatName" class="form-control" type="text" placeholder="<?php _e('Enter '.$default_labels['dc'].' Name','wp-support-plus-responsive');?>" >
	<button class="btn btn-success" onclick="createNewFaqCategory();"><?php _e('Create New '.$default_labels['dc'],'wp-support-plus-responsive');?></button>
</div>
<div id="editFaqCategoryContainer">
	...
	<button onclick="updateFaqCategory();" class="btn btn-success"><?php _e('Update '.$default_labels['dc'],'wp-support-plus-responsive');?></button>
</div>
