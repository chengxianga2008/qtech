...
$priority=__($priority,'wp-support-plus-responsive');
 ...
 */
if ( !isset( $_POST['agent_silent_create'] ) ) {
	if($wpsp_et_create_new_ticket['enable_success']){
		$subject_user = '['.__($advancedSettings['ticket_label_alice'][1],'wp-support-plus-responsive').' '.$advancedSettings['wpsp_ticket_id_prefix'].$ticket_id.'] ' . $et_success_subject;
		$body_user='';
...
if($wpsp_et_create_new_ticket['staff_to_notify']['administrator']){
	if($administrator_emails && !$to){
		...
		foreach ($administrator_emails as $admin_email){
			if($user_email != $admin_email && !(array_search($admin_email, $piping_emails)>-1)){
...
if($wpsp_et_create_new_ticket['staff_to_notify']['supervisor']){
	...
	if($supervisors_emails && !$to){
		...
		foreach ($supervisors_emails as $supervisor_email){
			if($user_email!=$supervisor_email && !(array_search($supervisor_email, $piping_emails)>-1)){
...
if($wpsp_et_create_new_ticket['staff_to_notify']['all_agents']){
	...
	if($agents_emails && !$to){
		...
		foreach ($agents_emails as $agents_email){
			if($user_email!=$agents_email && !(array_search($agents_email, $piping_emails)>-1)){
...
$subject='['.__($advancedSettings['ticket_label_alice'][1],'wp-support-plus-responsive').' '.$advancedSettings['wpsp_ticket_id_prefix'].$ticket_id.'] '.$et_success_staff_subject;
$body='';
