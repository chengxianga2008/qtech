...
$sql="select subject,type,status,cat_id,priority,created_by,guest_name,guest_email,extension_meta
...
?>
<button class="btn btn-primary changeTicketSubBtn" onclick="backToTicketFromIndisual();"><?php _e($advancedSettings['ticket_label_alice'][17],'wp-support-plus-responsive');?></button>
<button class="btn btn-primary changeTicketSubBtn" onclick="getChangeTicketStatus(<?php echo $_POST['ticket_id'];?>);"><?php _e('Change Status','wp-support-plus-responsive');?></button>
...
<?php }?>
<?php if($current_user->has_cap('manage_support_plus_ticket')){?>
<button class="btn btn-primary wpsp_ticket_nav_btn" id="psmwpsp_canned"  type="submit" data-target="#psmwpsp_loader" onclick="cannedrep();"><?php _e('Canned Reply','wp-support-plus-responsive');?></button>
<?php }?>
...
if($ticket->extension_meta){
    ...
    if($extension_meta[0]==1){ //woocommerce
        if($extension_meta[1]==1){ //product
            ...
            echo "<b>".__('Order','wp-support-plus-responsive').":</b> ".$order_title."<br>";
            ?>
