 ...
 */
$sql="select t.id,t.type,t.subject,t.status,c.name as category,t.assigned_to,t.priority,t.created_by,t.guest_name,t.agent_created,cs.color,cp.color as pcolor,".$customFieldSql."
...
switch($_POST['filter_by_selection'])
{
	...
	case 'created_by':
		if($_POST['filter_by_search']!=''){
			...
			$sql1 = "SELECT * FROM " . $wpdb->prefix . "users WHERE 1=1 AND (user_login LIKE '%" . $term . "%' OR user_email LIKE '%" . $term . "%' OR display_name LIKE '%" . $term . "%')";
...
$total_pages=ceil($wpdb->num_rows/$_POST['filter_by_no_of_ticket']);
...
<div class="table-responsive">
	<table class="table table-striped table-hover">
			...
			if($backend_ticket_field_value)
			{
				...
				else
				{
					switch($backend_ticket_field_key){
						...
						break;
						case 'st': ?><th id="st" onclick="sort_tickets('st');"><?php _e('Status','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='st'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'sb': ?><th id="sb" onclick="sort_tickets('sb');"><?php _e($default_labels['ds'],'wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='sb'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'rb': ?><th id="rb" onclick="sort_tickets('rb');"><?php _e('Raised By','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='rb'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'ty': ?><th id="ty" onclick="sort_tickets('ty');"><?php _e('Type','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='ty'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'ct': ?><th id="ct" onclick="sort_tickets('ct');"><?php _e($default_labels['dc'],'wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='ct'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'at': ?><th id="at" onclick="sort_tickets('at');"><?php _e('Assigned to','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='at'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'pt': ?><th id="pt" onclick="sort_tickets('pt');"><?php _e($default_labels['dp'],'wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='pt'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'ut': ?><th id="ut" onclick="sort_tickets('ut');"><?php _e('Updated','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='ut'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'cdt': ?><th id="cdt" onclick="sort_tickets('cdt');"><?php _e('Date Created','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='cdt'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
						case 'udt': ?><th id="udt" onclick="sort_tickets('udt');"><?php _e('Date Updated','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='udt'){
								?><span class="dashicons"></span><?php
							}
						...
						break;
                                                case 'acd': ?><th id="acd" onclick="sort_tickets('acd');"><?php _e('Agent Created','wp-support-plus-responsive');?><?php
							if($order=="" && $sortby=='acd'){
								?><span class="dashicons"></span><?php
							}
	  ...
	  foreach ($tickets as $ticket){
		...
		*/
		...
		//echo "<tr class='".$status_color."' style='cursor:pointer;'  onclick='if(link)openTicket(".$ticket->id.");'>";
		echo "<tr style='cursor:pointer;'  onclick='if(link)openTicket(".$ticket->id.");'>";
		echo "<td onmouseover='link=false;' onmouseout='link=true;'><input id='".$ticket->id."' type='checkbox' class='bulk_action_checkbox' onchange='wpspCheckBulkActionVisibility();' name='selected[]' value='".$ticket->id."'/></td>";
					...
					else
					{
						switch($backend_ticket_field_key){
							...
							case 'sb':$str_dots=""; 
								...
								echo "<td>".substr(stripcslashes(htmlspecialchars_decode($ticket->subject,ENT_QUOTES)), 0,$subCharLength['backend']).$str_dots."</td>";
							break;
							case 'rb': echo "<td>".__($raised_by,'wp-support-plus-responsive')."</td>";
							break;
							case 'ty': echo "<td>".__(ucfirst($ticket->type),'wp-support-plus-responsive')."</td>";
							break;
							case 'ct': echo "<td>".__($ticket->category,'wp-support-plus-responsive')."</td>";
							break;
							case 'at': echo "<td>".__($agent_name,'wp-support-plus-responsive')."</td>";
							break;
							case 'pt': echo "<td><span class='label label-".$priority_color."' style='font-size: 13px;background-color:".$priority_color."'>".__(ucfirst($ticket->priority),'wp-support-plus-responsive')."</span></td>";
							break;
							...
							break;
                                                        case 'acd': echo "<td>".__($agent_created,'wp-support-plus-responsive')."</td>";
							break;
						}
					}
				}
	...
	<ul class="pager" style="<?php echo ($total_pages==0)? 'display: none;':'';?>">
	  <li class="previous <?php echo $prev_class;?>"><a href="javascript:load_prev_page(<?php echo $prev_page_no;?>,'<?php echo $sortby;?>','<?php echo $order;?>');">&larr; <?php _e('Newer','wp-support-plus-responsive');?></a></li>
	  ...
	  <li class="next <?php echo $next_class;?>"><a href="javascript:load_next_page(<?php echo $next_page_no;?>,'<?php echo $sortby;?>','<?php echo $order;?>');"><?php _e('Older','wp-support-plus-responsive');?> &rarr;</a></li>
	</ul>
