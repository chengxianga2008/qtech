...
<div id="priorityDisplayTableContainer" class="table-responsive">
	...
	<thead>
	  <tr>
	    ...
	    <th><?php _e('Color','wp-support-plus-responsive');?></th>
	    <th><?php _e('Save','wp-support-plus-responsive');?></th>
	  </tr>
	</thead>
	<tbody>
	  	...
	  	<tr>
			...
			<td style="display:none;"><?php echo $priority->id;?></td>
	  		<td>
				...
				if(!in_array($priority->name,$priority_names)){
				?>
				<img onclick="delete_custom_priority(<?php echo $priority->id;?>);" style="cursor: pointer;" title="<?php _e('Delete','wp-support-plus-responsive');?>" src="<?php echo WCE_PLUGIN_URL;?>asset/images/delete.png" >
				<?php 
				}
				?>
			</td>
	  		...
	  		<td><img onclick="save_custom_priority_color(<?php echo $priority->id;?>);" style="cursor: pointer;" title="<?php _e('Update','wp-support-plus-responsive');?>" src="<?php echo WCE_PLUGIN_URL;?>asset/images/save.png" > <span class="custom-priority-color-saved" id="custom-priority-color-saved-<?php echo $priority->id;?>">Saved!</span></td>
	  	</tr>
	  <?php }?>
	</tbody>
...
<input type="button" onclick="setCustomPriorityOrder();" class="btn btn-success" value="<?php _e('Save '.$default_labels['dp'].' Order','wp-support-plus-responsive');?>">
<?php if(!$total_priorities){?>
	<div style="width: 100%;text-align: center;"><?php _e('No Custom '.$default_labels['dp'].' Found','wp-support-plus-responsive');?></div>
	<hr>
<?php }?>
<div id="add_custom_priority_container">
	<h4><?php _e('Add New Priority','wp-support-plus-responsive');?></h4>
	<table>
		<tr>
			<td><?php _e($default_labels['dp'].' Name','wp-support-plus-responsive');?>&nbsp;:&nbsp;</td>
			<td><input type="text" id="custom_priority_text" value="" ></td>
		</tr>
		<tr>
			<td><?php _e($default_labels['dp'].' Color','wp-support-plus-responsive');?>&nbsp;:&nbsp;</td>
			<td><input type="text" id="custom_priority_color" value="" class="wp-support-plus-color-picker" ></td>
		</tr>
	</table><br>
	<button class="btn btn-success" onclick="create_custom_priority();" ><?php _e('Create New '.$default_labels['dp'],'wp-support-plus-responsive');?></button>
</div>
...
<div id="editCustomPriority">
	...
	<button onclick="updateCustomPriority();" class="btn btn-success"><?php _e('Update','wp-support-plus-responsive');?></button>
</div>
