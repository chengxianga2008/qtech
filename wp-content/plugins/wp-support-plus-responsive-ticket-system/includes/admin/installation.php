...
if ( current_filter() != 'plugins_loaded' || $installed_version != WPSP_VERSION ) {
	...
	dbDelta( $sql );
        if( get_option( 'wpsp_add_index_ticket' ) === false ){
        ...
        update_option('wpsp_add_index_ticket',TRUE);
	// wpsp_ticket_thread
	...
	dbDelta( $sql );
        if( get_option( 'wpsp_add_index_ticket_thread' ) === false ){
            $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket_thread ADD INDEX  'ticket_id' (  'ticket_id' )");
        }
        update_option('wpsp_add_index_ticket_thread',TRUE);
	// wpsp_attachments
	...
	dbDelta( $sql );
        ...
        if( get_option( 'wpsp_add_index_canned_reply' ) === false ){
            $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_canned_reply ADD INDEX  'uID' (  'uID' )");
        }
        update_option('wpsp_add_index_canned_reply',TRUE);
	...
	if( get_option( 'wpsp_default_status_priority_names' ) === false ){
        ...
        if( get_option( 'wpsp_add_index_custom_status' ) === false ){
            $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_custom_status ADD INDEX  'name' (  'name' )(255)");
        }
        update_option('wpsp_add_index_custom_status',TRUE);
	...
	dbDelta( $sql );
        ...
        if( get_option( 'wpsp_default_status_priority_names' ) === false ){
            ...
            if(!$defaultPriority){
                    $wpdb->insert($wpdb->prefix.'wpsp_custom_priority',array('name'=>__('Normal','wp-support-plus-responsive'),'color'=>'#5cb85c'));
                    $wpdb->insert($wpdb->prefix.'wpsp_custom_priority',array('name'=>__('High','wp-support-plus-responsive'),'color'=>'#d9534f'));
                    $wpdb->insert($wpdb->prefix.'wpsp_custom_priority',array('name'=>__('Medium','wp-support-plus-responsive'),'color'=>'#f0ad4e'));
                    $wpdb->insert($wpdb->prefix.'wpsp_custom_priority',array('name'=>__('Low','wp-support-plus-responsive'),'color'=>'#5bc0de'));
            }
        }
	
	// update wp_support_plus_version option to plugin version
	update_option( 'wp_support_plus_version', WPSP_VERSION );
	if( get_option( 'wpsp_add_index_custom_priority' ) === false ){
            $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_custom_priority ADD INDEX  'name' (  'name' )(255)");
        }
        update_option('wpsp_add_index_custom_priority',TRUE);
	...
	if(!isset($advancedSettings['ticket_label_alice']))
	{
		$advancedSettings['ticket_label_alice']=array(
                    1=>__('Ticket','wp-support-plus-responsive'),
                    2=>__('Tickets','wp-support-plus-responsive'),
                    3=>__('Create New Ticket','wp-support-plus-responsive'),
                    4=>__('Ticket List Settings','wp-support-plus-responsive'),
                    5=>__('Create Ticket As','wp-support-plus-responsive'),
                    6=>__('Make Ticket Public','wp-support-plus-responsive'),
                    7=>__('Submit Ticket','wp-support-plus-responsive'),
                    8=>__('Edit Ticket','wp-support-plus-responsive'),
                    9=>__('Reply Ticket','wp-support-plus-responsive'),
                    10=>__('Delete Ticket','wp-support-plus-responsive'),
                    11=>__('Ticket Type','wp-support-plus-responsive'),
                    12=>__('No. of Tickets','wp-support-plus-responsive'),
                    13=>__('Ticket Creator','wp-support-plus-responsive'),
                    14=>__('Create Ticket Success Email','wp-support-plus-responsive'),
                    15=>__('Delete Ticket Notification Email','wp-support-plus-responsive'),
                    16=>__('New Ticket From Thread','wp-support-plus-responsive'),
                    17=>__('Back to Tickets','wp-support-plus-responsive'),
                    18=>__('Backend Ticket List Fields','wp-support-plus-responsive'),
                    19=>__('Frontend Ticket List Fields','wp-support-plus-responsive'),
                    20=>__('No Tickets Found','wp-support-plus-responsive')
                );
		update_option('wpsp_advanced_settings',$advancedSettings);
	}
	...
	if(!isset($advancedSettings['wpsp_reply_form_position']))
	{
        ...
        if(!isset($generalSettings['close_ticket_btn_status_val'])){
            ...
            $generalSettings['close_btn_alice']=__('Close Ticket','wp-support-plus-responsive');
            update_option('wpsp_general_settings',$generalSettings);
        }
	...
	if(!isset($advancedSettingsFieldOrder['fields_order'])){
	...
	if(!isset($advancedSettingsFieldOrder['default_fields_label'])){
        ...
        if(!isset($advancedSettingsFieldOrder['wpsp_default_value_of_subject'])){
            $advancedSettingsFieldOrder['wpsp_default_value_of_subject']=__("NA", 'wp-support-plus-responsive' );
            update_option('wpsp_advanced_settings_field_order',$advancedSettingsFieldOrder);
        }
	/***************************************************************************************/
	...
	if( get_option( 'wpsp_et_create_new_ticket' ) === false ) {
		$templates=array(
				'customer_name' => __("Customer Name", 'wp-support-plus-responsive' ),
				'customer_email' => __("Customer Email", 'wp-support-plus-responsive' ),
				'ticket_id' => __("Ticket ID", 'wp-support-plus-responsive' ),
				'ticket_subject' => __("Ticket Subject", 'wp-support-plus-responsive' ),
				'ticket_description' => __("Ticket Description", 'wp-support-plus-responsive' ),
				'ticket_category' => __("Ticket Category", 'wp-support-plus-responsive' ),
				'ticket_priotity' => __("Ticket Priority", 'wp-support-plus-responsive' )
		);
		foreach($customFields as $field){
			$ticket_fields_list_backend=array_merge($ticket_fields_list_backend,array($field->id));
		...
		$wpsp_et_create_new_ticket=array(
				'enable_success'=>'1',
				'success_subject'=>__("Your Ticket has been created successfully", 'wp-support-plus-responsive' ),
        ...
        if(!isset($wpsp_et_create_new_ticket['templates']['ticket_url'])){
            $wpsp_et_create_new_ticket['templates']['ticket_url']=__("Ticket URL", 'wp-support-plus-responsive' );
            update_option('wpsp_et_create_new_ticket',$wpsp_et_create_new_ticket);
        }
        $wpsp_et_create_new_ticket['templates']['ticket_description']=__("Ticket Description", 'wp-support-plus-responsive' );
        update_option('wpsp_et_create_new_ticket',$wpsp_et_create_new_ticket);
	if(!isset($wpsp_et_create_new_ticket['templates']['time_created'])){
            $wpsp_et_create_new_ticket['templates']['time_created']=__("Ticket Created", 'wp-support-plus-responsive' );
            update_option('wpsp_et_create_new_ticket',$wpsp_et_create_new_ticket);
        }
        if(!isset($wpsp_et_create_new_ticket['templates']['agent_created'])){
            $wpsp_et_create_new_ticket['templates']['agent_created']=__("Agent Created", 'wp-support-plus-responsive' );
            update_option('wpsp_et_create_new_ticket',$wpsp_et_create_new_ticket);
        }
	...
	*/
	...
	if( get_option( 'wpsp_et_reply_ticket' ) === false ) {
		$templates=array(
				'reply_by_name' => __("Reply By Name", 'wp-support-plus-responsive' ),
				'reply_by_email' => __("Reply By Email", 'wp-support-plus-responsive' ),
				'ticket_status' => __("Ticket Status", 'wp-support-plus-responsive' ),
				'customer_name' => __("Customer Name", 'wp-support-plus-responsive' ).__("(ticket creator)", 'wp-support-plus-responsive' ),
				'customer_email' => __("Customer Email", 'wp-support-plus-responsive' ).__("(ticket creator)", 'wp-support-plus-responsive' ),
				'ticket_id' => __("Ticket ID", 'wp-support-plus-responsive' ),
				'ticket_subject' => __("Ticket Subject", 'wp-support-plus-responsive' ),
				'reply_description' => __("Reply Description", 'wp-support-plus-responsive' ),
				'ticket_category' => __("Ticket Category", 'wp-support-plus-responsive' ),
				'ticket_priotity' => __("Ticket Priority", 'wp-support-plus-responsive' )
		);
		foreach($customFields as $field){
        ...
        if(!isset($wpsp_et_reply_ticket['templates']['ticket_url'])){
            $wpsp_et_reply_ticket['templates']['ticket_url']=__("Ticket URL", 'wp-support-plus-responsive' );
            update_option('wpsp_et_reply_ticket',$wpsp_et_reply_ticket);
        }
       
        $wpsp_et_reply_ticket['templates']['reply_description']=__("Reply Description", 'wp-support-plus-responsive' );
        update_option('wpsp_et_reply_ticket',$wpsp_et_reply_ticket);
        
        if(!isset($wpsp_et_reply_ticket['templates']['time_created'])){
            $wpsp_et_reply_ticket['templates']['time_created']=__("Ticket Created", 'wp-support-plus-responsive' );
            update_option('wpsp_et_reply_ticket',$wpsp_et_reply_ticket);
        }
	/*
	...
	if( get_option( 'wpsp_et_change_ticket_status' ) === false ) {
        ...
        if(!isset($wpsp_et_change_ticket_status['templates'])){
            $templates=array(
                ...
                'ticket_category' => __("Ticket Category", 'wp-support-plus-responsive' ),
                'ticket_priotity' => __("Ticket Priority", 'wp-support-plus-responsive' ),
                'ticket_url'=>__("Ticket URL", 'wp-support-plus-responsive' ),
                'updated_by'=>__("User who changed status", 'wp-support-plus-responsive' )
            );
            foreach($customFields as $field){
                $templates['cust'.$field->id]=$field->label;
            }
        ...
        if(!isset($wpsp_et_change_ticket_status['templates']['time_created'])){
            $wpsp_et_change_ticket_status['templates']['time_created']=__("Ticket Created", 'wp-support-plus-responsive' );
            update_option('wpsp_et_change_ticket_status',$wpsp_et_change_ticket_status);
        }
	/*
	...
	if( get_option( 'wpsp_et_change_ticket_assign_agent' ) === false ) {
        ...
        if(!isset($wpsp_et_change_ticket_assign_agent['templates'])){
            $templates=array(
                ...
                'ticket_priotity' => __("Ticket Priority", 'wp-support-plus-responsive' ),
                'ticket_url'=>__("Ticket URL", 'wp-support-plus-responsive' ),
                'updated_by'=>__("User who assigned ticket", 'wp-support-plus-responsive' ),
                'old_assigned_to'=>__("User to whom assigned ticket before", 'wp-support-plus-responsive' ),
                'new_assigned_to'=>__("User to whom assigned ticket now", 'wp-support-plus-responsive' )
            );
            foreach($customFields as $field){
                $templates['cust'.$field->id]=$field->label;
            }
        ...
        if(!isset($wpsp_et_change_ticket_assign_agent['templates']['time_created'])){
            $wpsp_et_change_ticket_assign_agent['templates']['time_created']=__("Ticket Created", 'wp-support-plus-responsive' );
            update_option('wpsp_et_change_ticket_assign_agent',$wpsp_et_change_ticket_assign_agent);
        }
	/*
    ...
    if(!isset($wpsp_et_delete_ticket['templates']['time_created'])){
            $wpsp_et_delete_ticket['templates']['time_created']=__("Ticket Created", 'wp-support-plus-responsive' );
            update_option('wpsp_et_delete_ticket',$wpsp_et_delete_ticket);
        }
	/*
