...
?>
<h3><?php _e($advancedSettings['ticket_label_alice'][3],'wp-support-plus-responsive');?></h3>
<form id="frmCreateNewTicket" name="frmCreateNewTicket">
		...
		if(in_array($field_id,$advancedSettingsFieldOrder['display_fields']))
		{
					...
					else
					{
						switch($field->field_type){
							...
							break;
                                                        case '6': ?>
                                                                <span class="label label-info" style="font-size: 13px;"><?php echo $field->label;?></span><br> 
                                                                <input id="cust<?php echo $field->id;?>" class="wpsp_datepicker" type="text"  name="cust<?php echo $field->id;?>" style="width: 95%; margin-top: 10px;" placeholder="<?php _e('Click here to select the date','wp-support-plus-responsive');?>"/><br><br>
                                                        <?php
                                                        break;
						}
					}
			...
			else
			{
				switch($field_id)
				{
					case 'dn':
						if ($_POST['backend']){?>
							<span class="label label-info wpsp_title_label"><?php _e($advancedSettings['ticket_label_alice'][5],'wp-support-plus-responsive');?></span><br><br>
							<select id="create_ticket_user_type" name="create_ticket_user_type" class="hide_fields_support_plus" onchange="change_user_type();">
								<option value="user"><?php _e('Registered','wp-support-plus-responsive');?></option>
							...
							<div id="user_type_user">
                                                            ...
                                                            <button type="button" class="btn btn-primary" id="searchUserModal" onclick="getSearchUserForm();"><?php _e('Change User','wp-support-plus-responsive');?></button>
                                                            <br>
							</div>
							<br>
							<div id="user_type_guest" style="display:none">
							<span class="label label-info wpsp_title_label"><?php _e($default_labels['dn'],'wp-support-plus-responsive');?></span><code>*</code><br>
							...
							<span class="label label-info wpsp_title_label"><?php _e($default_labels['de'],'wp-support-plus-responsive');?></span><code>*</code><br>
					...
					case 'dd': ?>
                                                <?php if(!$_POST['backend']){?>
                                                    <span class="label label-info wpsp_title_label"><?php _e($default_labels['dd'],'wp-support-plus-responsive');?></span><code>*</code><br>
                                                <?php } else { ?>
                                                    <table>
                                                        <tr>
                                                            ...
                                                            <td style="padding:10px 10px 10px 0px;">
                                                                <button type="button" class="btn btn-primary"  id="psmwpsp_canned"  onclick="cannedrep();"><?php _e('Canned Reply','wp-support-plus-responsive');?></button>
                                                            </td>
                                                        <tr>
                                                    </table>
                                                <?php }?>
					...
					case 'dc': ?>
						<div>
							<span class="label label-info wpsp_title_label hide_fields_support_plus"><?php _e($default_labels['dc'],'wp-support-plus-responsive');?></span><code>*</code><br>
								...
								foreach ($categories as $category){
	                                                            echo '<option value="'.$category->id.'">'.__(stripcslashes($category->name),'wp-support-plus-responsive').'</option>';
								}
								?>
							    </select><br><br>
                                                </div>
					...
					case 'dp': ?>
						<div>
							<span class="label label-info wpsp_title_label hide_fields_support_plus"><?php _e($default_labels['dp'],'wp-support-plus-responsive');?></span><code>*</code><br>
	...
	if($generalSettings['enable_user_selection_public_private']==1 || $_POST['backend'])
	{ ?>
	...
	<span class="label label-info" style="font-size: 13px;"><?php _e($advancedSettings['ticket_label_alice'][6],'wp-support-plus-responsive');?></span><br><br/>
<?php 	}else
	{?>
...
<?php	}?>
	 ...
	 * Update 20 - suppress owner notification for tickets created by agent
	 */
	if ( $wpsp_et_create_new_ticket['enable_success'] && $_POST['backend'] && ( $current_user->has_cap( 'manage_support_plus_ticket' ) || $current_user->has_cap( 'manage_support_plus_agent' ) ) ) {
		?>
                <input type="checkbox" id="agent_silent_create" name="agent_silent_create" value="1" />	
		<span class="label label-info" style="font-size: 13px;"><?php _e('Don\'t Notify Owner','wp-support-plus-responsive');?></span><br><br/>	
		<?php
	}
	/* END CLOUGH I.T. SOLUTIONS MODIFICATION
	 */
  ...
  <div id="modal-dialog">
    <div id="modal-content">
      <div id="modal-header">
        ...
        <h4 class="title" id="myModalLabel"><?php _e('Select User','wp-support-plus-responsive');?></h4>
      </div>
      <div id="body">
        <?php include( WCE_PLUGIN_DIR.'includes/admin/selectRegisteredUser.php' );?>
      </div>
      <div id="footer">
          <button type="button" class="btn btn-default" onclick="closep();"><?php _e('Close','wp-support-plus-responsive');?></button>
      </div>
    </div>
  </div>
</div>
    ...
    <div  id="myModal">
     <h4 id="myModalLabel"><?php _e('Canned Reply','wp-support-plus-responsive');?></h4>
        ...
        <div id="popup">
                ...
                <div>
                    <button type="button" class="btn btn-default" id="closebtn" onclick="closepopup();" style="float:right"><?php _e('Close','wp-support-plus-responsive');?></button>
                </div>
        </div>
</div>
