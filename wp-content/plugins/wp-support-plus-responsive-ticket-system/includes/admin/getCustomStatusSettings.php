	  	...
	  	<tr>
			...
			<td style="display:none;"><?php echo $status->id;?></td>
	  		<td>
				...
				if(!in_array($status->name,$status_names)){
				?>
				<img onclick="delete_custom_status(<?php echo $status->id;?>);" style="cursor: pointer;" title="<?php _e('Delete','wp-support-plus-responsive');?>" src="<?php echo WCE_PLUGIN_URL;?>asset/images/delete.png" >
				<?php 
				}
				?>
			</td>
			...
			?>
	  		...
	  		<td><img onclick="save_custom_status_color(<?php echo $status->id;?>);" style="cursor: pointer;" title="<?php _e('Update','wp-support-plus-responsive');?>" src="<?php echo WCE_PLUGIN_URL;?>asset/images/save.png" > <span class="custom-status-color-saved" id="custom-status-color-saved-<?php echo $status->id;?>">Saved!</span></td>
	  		<?php
	  		/* END CLOUGH I.T. SOLUTIONS MODIFICATION
			*/
			?>
...
<input type="button" onclick="setCustomStatusOrder();" class="btn btn-success" value="<?php _e('Save Status Order','wp-support-plus-responsive');?>">
<?php if(!$total_statusses){?>
	<div style="width: 100%;text-align: center;"><?php _e('No Custom Status Found','wp-support-plus-responsive');?></div>
	<hr>
<?php }?>
<div id="add_custom_status_container">
	<h4><?php _e('Add New Status','wp-support-plus-responsive');?></h4>
	<table>
		<?php
		/* BEGIN CLOUGH I.T. SOLUTIONS MODIFICATION
		 * Update 1 - Change Custom Status Color
		...
		?>
		<tr>
			<td><?php _e('Status Name','wp-support-plus-responsive');?>&nbsp;:&nbsp;</td>
			<td><input type="text" id="custom_status_text" value="" ></td>
		</tr>
		<tr>
			<td><?php _e('Status Color','wp-support-plus-responsive');?>&nbsp;:&nbsp;</td>
	...
	<button class="btn btn-success" onclick="create_custom_status();" ><?php _e('Create New Status','wp-support-plus-responsive');?></button>
</div>
...
<div id="editCustomStatus">
	...
	<button onclick="updateCustomStatus();" class="btn btn-success"><?php _e('Update','wp-support-plus-responsive');?></button>
</div>
