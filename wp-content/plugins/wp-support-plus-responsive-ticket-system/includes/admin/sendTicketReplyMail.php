...
$etCategoryName=__($etCategoryName,'wp-support-plus-responsive');
$description=stripcslashes($_POST['replyBody']);
$priority=__($ticket->priority,'wp-support-plus-responsive');
...
$ticketStatus=__(ucfirst($ticket->status),'wp-support-plus-responsive');
...
$subject='['.__($advancedSettings['ticket_label_alice'][1],'wp-support-plus-responsive').' '.$advancedSettings['wpsp_ticket_id_prefix'].$ticket->id.'] '.$et_success_staff_subject;
$body='';
...
if($wpsp_et_reply_ticket['notify_to']['assigned_agent'] && $ticket->assigned_to != '0'){
	...
	if (!$to){
		...
		foreach ($assigned_users as $user){
			$userdata=get_userdata($user);
			if($ticket_user_email!=$userdata->user_email && $ticket_user_email!=$customerEmail && !(array_search($userdata->user_email, $piping_emails)>-1)){
...
if($wpsp_et_reply_ticket['notify_to']['administrator']){
	if($administrator_emails && !$to){
		...
		foreach ($administrator_emails as $admin_email){
			if($ticket_user_email != $admin_email && $ticket_user_email!=$customerEmail && !(array_search($admin_email, $piping_emails)>-1)){
...
if($wpsp_et_reply_ticket['notify_to']['supervisor']){
	...
	if($supervisors_emails && !$to){
		...
		foreach ($supervisors_emails as $supervisor_email){
			if($ticket_user_email!=$supervisor_email && $ticket_user_email!=$customerEmail && !(array_search($supervisor_email, $piping_emails)>-1)){
...
if($wpsp_et_reply_ticket['notify_to']['all_agents']){
	...
	if($agents_emails && !$to){
		...
		foreach ($agents_emails as $agents_email){
			if($ticket_user_email!=$agents_email && $ticket_user_email!=$customerEmail && !(array_search($agents_email, $piping_emails)>-1)){
