...
<a href="<?php echo admin_url('admin.php?page=wp-support-plus-Canned-Reply&type=add');?>" class="btn btn-primary"><?php _e("+ Add New",'wp-support-plus-responsive');?></a>
<br><br>
<?php 
global $wpdb;
global $current_user;
...
<table class="table table-striped table-hover">
	  <tr>
		  ...
		  <th><?php _e('Title','wp-support-plus-responsive');?></th>
                  <th><?php _e('Created By','wp-support-plus-responsive');?></th>
	  	...
	  	<tr>
	  		...
	  		<td>
                            <?php if($can->uID==$current_user->ID){?>
                                <a href="<?php echo admin_url('admin.php?page=wp-support-plus-Canned-Reply&type=edit&id='.$can->id);?>" class="btn btn-info"><?php _e('Edit','wp-support-plus-responsive');?></a>
                                <button class="btn btn-danger" onclick="deletcanned(<?php echo $can->id;?>);"><?php _e('Delete','wp-support-plus-responsive');?></button>
                                <button class="btn btn-info" id="showcanned" data-toggle="modal" data-target="#Modal" onclick="setCurrentCannedId(<?php echo $can->id;?>,'<?php echo $can->sid;?>');"><?php _e('Share','wp-support-plus-responsive');?></button>
                            <?php }?>
                        </td>
	  	</tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel"><?php _e('Canned Reply','wp-support-plus-responsive');?></h4>
            </div>
            <div class="modal-body" id="mydiv">
                <?php 
                  ...
                  $canned = $wpdb->get_results( $sql );
                ?>
                <?php _e('Users','wp-support-plus-responsive');?>:<br>
		    ...
		    <?php 
                ...
                <div class="modal-footer">
                    <button class="btn btn-info" id="share" onclick="setShareCanned();"><?php _e('Share','wp-support-plus-responsive');?></button>
